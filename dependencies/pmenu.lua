...
local _intX, _intY = .224, .275 --.24, .275
local _intW, _intH = .213, .035 --.223, .035
local spriteW, spriteH = .213, .0875 --.223, .0675
local PMenu = {}
...
function PMenu:CreateMenu(tableMenu, tempData)
	...
	if not self.IsVisible and tableMenu then
		...
		for k,v in pairs(tableMenu.Base) do
			if k == "Header" then 
				RequestStreamedTextureDict(v[1]) 
				SetStreamedTextureDictAsNoLongerNeeded(v[1]) 
			end
			self.Base[k] = v
		end
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	elseif slideExist or button.ask or button.slidename then
		...
		if (max and type(max) == "number" and max > 0 or type(max) == "table" and #max > 0) or not slideExist then
			...
			local textX = (not boolSelected or button.ask) and -.007 or - .0135 -- power tu en était la 
...
function PMenu:DrawHeader(intCount)
	...
	DrawText2(0, stringCounter, .275, self.Width - rectW / 1.9 + .11, self.Height - _intH, color_white, true, 2)

	if currentMenu and currentMenu.charCreator then

		local spriteW, spriteH = .214, .21
...
function PMenu:DrawHelpers2(tableButtons)
	local Help = self.Base.Des or self.Menu[self.Data.currentMenu] and self.Menu[self.Data.currentMenu].Des or tableButtons[self.Pag[3]] and tableButtons[self.Pag[3]].Des
    if Help then
		local intH, scale = 0.0375, 0.275
		self.Height = self.Height - -0.020

		DrawRect(self.Width - _intW / 2, self.Height, _intW, 0.0025, 0, 0, 0, 255)

		local descText = MultilineFormat(Help, scale)
...
function PMenu:DrawHelpers(tableButtons)
	...
	local Kkxa = self.Base.Description2 or self.Menu[self.Data.currentMenu] and self.Menu[self.Data.currentMenu].Description2 or tableButtons[self.Pag[3]] and tableButtons[self.Pag[3]].Description2
	...
	if Kkxa then
		...
		local descText = MultilineFormat(Kkxa, scale)
		local Linecount = #stringsplit(descText, "\n")
		local nwintH = intH + (Linecount == 1 and 0 or ( (Linecount + 1) * 0.0075))
		self.Height = self.Height + intH / 2
		...
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 2, self.Height + nwintH / 2 - 0.015, _intW, nwintH, .0, 255, 255, 255, 255) 
		DrawText2(0, descText, scale, self.Width - _intW + .005, self.Height - 0.01, color_white)
		DrawSprite("commonmenu", "shop_arrows_upanddown", self.Width - _intW / 2, self.Height  - 0, .027, .03, 0.0, 255, 255, 255, 255)
	end
end
...
function PMenu:DrawExtra(tableButtons)
	...
	if button and button.advSlider ~= nil then
		...
		DrawText2(0, "Couleur disponibles", 0.275, self.Width - _intW / 2, self.Height, color_white, false, 0) --Variations disponibles
